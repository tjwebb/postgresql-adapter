...
const Adapter = {
    ...
    return Util.buildKnexJoinQuery (cxn, tableName, options)
      .then(result => {
        ...
        _.each(_.reject(options.joins, { select: false }), join => {
          ...
          _.each(result, row => {
            if (_.isArray(row[alias])) {
              row[alias] = _.map(row[alias], many => {
                const r = _.pick(many, join.select)
                return _.isEmpty(r) ? null : r
              })
            }
            else {
              const r = _.pick(row[alias], join.select)
              if (_.isEmpty(r)) {
                delete row[alias]
              }
              else {
  ...
  find (connectionName, tableName, options, cb) {
    ...
    if (options.select && !options.select.length) delete options.select
